...
===================================================================
--- src/server/api/stakeholders/GET.ts	original
+++ src/server/api/stakeholders/GET.ts	modified
@@ -1,562 +1,128 @@
 import type { Request, Response } from 'express';
+import { db } from '../../../server/db/client.js';
+import { stakeholders, stakeholderGroups, engagementActivities, feedbackRequests } from '../../../server/db/schema.js';
...
+    const allStakeholders = await db.select().from(stakeholders).orderBy(desc(stakeholders.createdAt));
...
-        },
-      ],
+    // Fetch all stakeholder groups from database
+    const allGroups = await db.select().from(stakeholderGroups).orderBy(stakeholderGroups.name);
...
-        },
-      ],
+    // Fetch engagement activities from database
+    const activities = await db.select().from(engagementActivities).orderBy(desc(engagementActivities.date)).limit(10);
...
+    const feedbacks = await db.select().from(feedbackRequests).orderBy(desc(feedbackRequests.deadline)).limit(10);
...
-        },
-      ],
+    // Calculate engagement metrics for each stakeholder
+    const stakeholdersWithMetrics = allStakeholders.map(stakeholder => ({
+      id: stakeholder.id,
...
+      focus_areas: stakeholder.focusAreas,
+      engagement_level: stakeholder.engagementLevel,
+      last_contact: stakeholder.lastContact,
+      projects: stakeholder.projects,
+      contributions: stakeholder.contributions,
+      contact_person: stakeholder.contactPerson,
+      contact_email: stakeholder.contactEmail,
...
+    const groupsWithMetrics = allGroups.map(group => ({
+      id: group.id,
+      name: group.name,
+      description: group.description,
+      member_count: group.memberCount,
+      engagement_score: group.engagementScore,
+      active_projects: group.activeProjects,
+      last_meeting: group.lastMeeting,
+      focus_sdgs: group.focusSdgs,
...
+      stakeholder_ids: activity.stakeholderIds,
...
+      stakeholder_ids: feedback.stakeholderIds,
+      responses_count: feedback.responsesCount,
...
+    const activeStakeholders = allStakeholders.filter(s => s.status === 'active').length;
+    const totalGroups = allGroups.length;
+    const totalActivities = activities.length;
+    const pendingFeedbacks = feedbacks.filter(f => f.status === 'open').length;
+
+    // Calculate average engagement
+    const avgEngagement = allStakeholders.reduce((sum, s) => {
+      const level = s.engagementLevel === 'high' ? 100 : s.engagementLevel === 'medium' ? 60 : 30;
+      return sum + level;
+    }, 0) / (totalStakeholders || 1);
+
+    res.json({
       success: true,
...
+      data: {
+        stakeholders: stakeholdersWithMetrics,
+        groups: groupsWithMetrics,
+        activities: formattedActivities,
+        feedbackRequests: formattedFeedbacks,
+        summary: {
